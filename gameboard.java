...
public class gameboard {
    ...
    // Establish initial state of board
    	...
    	for (int i = 0; i < 8; i++) {
		    ...
		    while (!hasHitEnd) {
				...
				if (startingRow > 7 || startingRow < 0 || startingColumn > 7 || startingColumn < 0 || board.get(startingRow).get(startingColumn) == " /") {
			    	hasHitEnd = true;
			    	tilesToFlip = null;
				} else if (board.get(startingRow).get(startingColumn) == currentPlayer.getName()) {
			    	hasHitEnd = true;
				} else {
		    ...
		    if (tilesToFlip != null) {
				for (int index = 0; index < tilesToFlip.size(); index++) {
			    	...
			    	board.get(flipThisTileRow).set(flipThisTileCol, currentPlayer.getName());
				}
